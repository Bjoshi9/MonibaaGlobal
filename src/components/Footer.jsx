...
import { NAVIGATION, COMPANY_INFO, SOCIAL_MEDIA } from '../utils/constants';
            ...
            <div className="flex space-x-4">
              {
                SOCIAL_MEDIA.map((social) => (
                  <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white transition-colors">
              ...
              {/* {Object.entries(SOCIAL_MEDIA).map(([key, {name, url, icon }]) => (
                <a key={key} href='{url}' target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white transition-colors">
                  <span className="sr-only">{name}</span>
                  <svg className={`h-6 w-6 ${icon}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d={icon} />
                  </svg>
                </a>
              ))} */}
